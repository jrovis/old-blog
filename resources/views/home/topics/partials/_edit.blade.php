    <form action="{{ route('p.update', [hashIdEncode($topic->id)]) }}" method="POST" class="ui form item-form" accept-charset="UTF-8">
        {{ csrf_field() }}
        {{ method_field('PUT') }}

        <div class="field {{ $errors->has('title') ? 'error' : '' }}">
            <input class="form-control" type="text" name="title" id="title-field" value="{{ $topic->title }}" required=""
                   placeholder="标题">
        </div>
        ...
        <div class="field">
               <textarea rows="15" id="editor" name="body" placeholder="请使用 Markdown 编写"
                         required="">{{ $topic->body_original }}</textarea>
        </div>
